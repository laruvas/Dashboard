  ...
  return (
    ...
    >
      ...
      <header className="flex-between">
        <Link to="/" className="flex flex-gap-3" style={{ alignItems: 'center', fontWeight: 700, fontSize: 16 }}>
          <span style={{
      ...
      <main style={{
        ...
        <div style={{ maxWidth: 520, textAlign: 'center' }}>
          ...
          <div className="flex flex-gap-3" style={{ justifyContent: 'center' }}>
            {isAuthenticated ? (
              <Button as="link" to="/dashboard" size="lg">{t('welcome.openDashboard')}</Button>
            ) : (
              <>
                <Button as="link" to="/login" size="lg">{t('welcome.signIn')}</Button>
                <Button as="link" to="/register" size="lg" variant="ghost">{t('welcome.createAccount')}</Button>
              </>
            )}
          </div>
        </div>
