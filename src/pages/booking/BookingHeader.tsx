    ...
    <>
      ...
      <div className="stepper">
        <div className={`step ${step === 1 ? 'active' : 'done'}`}>
          <span className="num">1</span> {t('booking.step.service')}
        </div>
        <span className="sep">·</span>
        <div className={`step ${step === 2 ? 'active' : step > 2 ? 'done' : ''}`}>
          <span className="num">2</span> {t('booking.step.dateTime')}
        </div>
        <span className="sep">·</span>
        <div className={`step ${step === 3 ? 'active' : step > 3 ? 'done' : ''}`}>
          <span className="num">3</span> {t('booking.step.details')}
        </div>
        <span className="sep">·</span>
        <div className={`step ${step === 4 ? 'active' : ''}`}>
          <span className="num">4</span> {t('booking.step.confirm')}
        </div>
      </div>
    </>
  )
