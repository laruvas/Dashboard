...
export function registerServiceRoutes(app) {
  app.get(
    ...
    asyncRoute(async (req, res) => {
      const rows = await dbAll('SELECT * FROM services WHERE providerId = ? ORDER BY id', [
        req.user.userId,
      ])
  ...
  app.get(
    ...
    asyncRoute(async (req, res) => {
      ...
      if (Number(row.providerId) !== req.user.userId)
  ...
  app.patch(
    ...
    asyncRoute(async (req, res) => {
      ...
      for (const key of allowed) {
        ...
        sets.push(`${key} = ?`)
        values.push(
          ['tag', 'name', 'description'].includes(key)
