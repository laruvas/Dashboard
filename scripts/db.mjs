...
const url = process.env.TURSO_DATABASE_URL || `file:${resolve(__dirname, '..', 'slottr.db')}`
...
async function migrate() {
  // libSQL doesn't run multiple statements in one execute() — split them.
  const statements = [
...
/* ============== Convenience helpers ==============
   libSQL returns rows as plain objects; we just need typed parsers for the
   JSON columns. None of these touch the DB themselves. */
...
/* ============== Tiny query helpers ==============
   libSQL returns { rows, columns, lastInsertRowid, ... } — these unwrap to the
   plain shapes our server.mjs uses. */
...
/** Execute a write; returns { lastInsertRowid, rowsAffected }. */
